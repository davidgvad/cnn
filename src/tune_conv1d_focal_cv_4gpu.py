...
def run_training_worker(args: argparse.Namespace) -> None:
    ...
    if parameter_count != FIXED_BACKBONE["expected_parameters"]:
        raise RuntimeError(
            "Conv1D parameter count changed: expected "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Multi-GPU, four-fold Conv1D focal beta/gamma tuning on KDDTrain+."
        )
    )
    ...
    settings = {
        "model": "Conv1D",
        "backbone": FIXED_BACKBONE,
        "input_representation": "121 semantically ordered features, shape (121, 1)",
    ...
    print("Conv1D focal-loss Stage-1 sweep")
    ...
    protocol = {
        "schema_version": SCHEMA_VERSION,
        "experiment_key": experiment_key,
        "title": "Conv1D class-balanced focal-loss four-fold tuning",
        "source_and_data_fingerprint": source_fingerprint,
    ...
    text_path.write_text(
        "Conv1D focal-loss Stage-1 sweep\n"
