"""Tune Transformer class-balanced focal loss with fixed four-fold OOF validation.

Default experiment
------------------
* Model: the fixed 110,661-parameter Transformer used by
  ``cnn_opt_1d_4gpu.py``.
...
def resolve_cuda_tokens(gpus: Sequence[str]) -> Dict[str, str]:
    """Map logical worker IDs onto an optional parent CUDA allocation."""
...
def run_training_worker(args: argparse.Namespace) -> None:
    ...
    if parameter_count != FIXED_BACKBONE["expected_parameters"]:
        raise RuntimeError(
            "Transformer parameter count changed: expected "
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--name-prefix",
        default="transformer_focal_stage1",
        type=str,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Multi-GPU, four-fold Transformer focal beta/gamma tuning on KDDTrain+."
        )
    )
    ...
    settings = {
        "model": "Transformer",
        "backbone": FIXED_BACKBONE,
        "input_representation": "121 ordered scalar feature tokens, shape (121, 1)",
    ...
    print("Transformer focal-loss Stage-1 sweep")
    ...
    print("Launcher: direct Python process (no Slurm)")
    ...
    protocol = {
        "schema_version": SCHEMA_VERSION,
        "experiment_key": experiment_key,
        "title": "Transformer class-balanced focal-loss four-fold tuning",
        "source_and_data_fingerprint": source_fingerprint,
    ...
    text_path.write_text(
        "Transformer focal-loss Stage-1 sweep\n"
    ...
    print("\n=== Ranked Transformer focal settings ===")
